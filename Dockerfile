...
RUN if [ "$enable_postgresql" = "true" ]; then \
    git -c http.sslVerify=false clone --depth 1 --branch pilotscope-postgresql https://github.com/jothamwong/pilotscope.git PilotScopePostgreSQL && \
...
RUN if [ "$enable_spark" = "true" ]; then \
    ...
    git -c http.sslVerify=false clone --depth 1 --branch pilotscope-spark https://github.com/jothamwong/pilotscope.git PilotScopeSpark && \
    cd ./PilotScopeSpark && \
    # Download Spark
